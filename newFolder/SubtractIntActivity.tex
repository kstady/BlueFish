...
\title{Subtracting Integers}   % Use xmlatex all -s to compile when SERVE refuses to work.
\author{Kelly Stady}
...
\[ \textrm{\textbf{Elevation Span}} = \textrm{Highest Elevation} - \textrm{Lowest Elevation} \]
...
\begin{tikzpicture}[scale=0.5]
    % Draw Vertical Axis
    \draw[ultra thick, <->] (0,-4) -- (0,10) node[above, yshift=5pt] {\Large\textbf{\emph{Elevation (ft)}}};

    % Sea Level Line - Scaled back slightly to \Large
    \draw[blue!60!black, ultra thick] (-1,0) -- (1.5,0) node[right, xshift=5pt] {\Large\textbf{\emph{Sea Level (0)}}};

    % Mount Everest Point
    \filldraw[black] (0,8) circle (5pt) node[right, xshift=8pt] {\Large\emph{Mt. Everest ($+29,032$)}};

    % Ayding Lake Point
    \filldraw[black] (0,-2) circle (5pt) node[right, xshift=8pt] {\Large\emph{Ayding Lake ($-505$)}};

    % Braces
    \draw [decorate, decoration={brace, amplitude=9pt, raise=5pt}, very thick] (0,-2) -- (0,0)
        node [midway, left=25pt] {\Large\textbf{\emph{505 ft}}};

    \draw [decorate, decoration={brace, amplitude=15pt, raise=5pt}, very thick] (0,0) -- (0,8)
        node [midway, left=25pt] {\Large\textbf{\emph{29,032 ft}}};

    % Total Span Arrow - Stacked on two lines with fixed line break
    \draw[<->, orange!80!black, line width=2pt] (-8,-2) -- (-8,8)
        node [midway, left=15pt, align=center] {
            \Large\textbf{\emph{Elevation}}\\
            \Large\textbf{\emph{Span}}
        };
