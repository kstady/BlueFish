...
\newpage
\textbf{Example} \ To find the sum $-4+(-3),$ begin with \textcolor{blue!70!black}{\textbf{four blue rods}} to represent the number $-4$ and \textcolor{blue!70!black}{\textbf{three blue rods}} to represent the number $-3.$  Since there are no orange-blue pairs to remove, 7 blue rods remain, so the answer is $-7.$
...
% --- SECTION 1: THE SETUP (Lined up vertically) ---
    \begin{scope}[shift={(-6.5,0)}]
        \node[font=\bfseries\small, negColor, anchor=west] at (-0.3, 2.2) {Setup};
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \node[black, font=\bfseries\tiny, align=center] at (0.9, -1.35) {No Pairs to Remove!};
...
% --- SECTION 1: THE SETUP (Lined up vertically) ---
    ...
    \begin{scope}[shift={(-5,0)}]
    \node[font=\bfseries\small, black!80, anchor=west] at (-0.3, 2.2) {\emph{Setup}};
...
% --- SECTION 2: Remove Pairs ---
    \begin{scope}[shift={(0,0)}]
    \node[font=\bfseries\small] at (0.75, 2.2) {\emph{Remove Pairs}};
...
% --- SECTION 3: THE RESULT ---
    \begin{scope}[shift={(5.5,0)}]
    \node[font=\bfseries\small] at (0.6, 2.2) {\emph{Result}};

% The Result Label with Ximera prompt
    \node[anchor=north, posColor, font=\bfseries] at (0.6, 1) {$\begin{prompt} = \answer{4} \end{prompt}$};
    \end{scope}
...
The use of colors in the Orange-Blue Rule is not necessary. Below is the first example redone using $+$ signs for positive numbers and $-$ signs for negative numbers.  You can also use things like different colored poker chips or pennies and dimes---the sky's the limit!  Try modifying the rule using your own way to represent positive and negative numbers.  Be creative!

%Trying to get vertical spacing!  See what works in the html

\bigskip   %Needs space above and below it to work

%\mbox{} \\[\medskipamount]   Too much spacing!

%\paragraph{Example III}

\textbf{Example} \ To find the sum $-5+3,$ begin with \textbf{five $-$ signs} to represent the number $-5$ and \textbf{three $+$ signs} to represent the number $3.$  Next remove all $+$ $-$ pairs.  Since two $-$ signs remain, the answer is $-2.$
