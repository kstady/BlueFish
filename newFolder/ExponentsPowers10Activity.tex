...
\textbf{Base} $\begin{prompt}=\answer{10}\end{prompt}$  \hspace{1in}  \textbf{Exponent} $\begin{prompt}=\answer{7}\end{prompt}$
...
\textbf{Base} $\begin{prompt}=\answer{4}\end{prompt}$  \hspace{1in}  \textbf{Exponent} $\begin{prompt}=\answer{5}\end{prompt}$

\end{problem}

\begin{problem} Identify the base and exponent of $\left(\frac{1}{7} \right)^2.$

\textbf{Base} $\begin{prompt}=\answer{\frac{1}{7}}\end{prompt}$  \hspace{1in}  \textbf{Exponent} $\begin{prompt}=\answer{2}\end{prompt}$
...
\begin{problem} Write the expression $\left(\frac{1}{5} \right)^3$ as a product without the exponent.

\begin{multipleChoice}
    \choice{$\left(\frac{1}{5} \right) \cdot 3$}
    \choice{$\left(\frac{1}{5} \right) \cdot \left(\frac{1}{5} \right)$ }
    \choice[correct]{$\left(\frac{1}{5} \right) \cdot \left(\frac{1}{5} \right) \cdot \left(\frac{1}{5} \right)$}
  \end{multipleChoice}
...
Below are the first three positive powers of 10.  Notice that the exponent in each power of 10 is the same as the number of zeros in the result.  In general, if $n$ is a positive integer, then the value of $10^n$ is the number 1 followed by $n$ zeros.  For example, below you can see that $10^3$ equals the number 1 followed by 3 zeros.
\begin{eqnarray*}
...
\begin{problem} Write the value of $10^6.$


\[ 10^6 \begin{prompt}=\answer{1,000,000 }\end{prompt} \]
...
in the power of 10.  Once you notice this relationship, you won't need to perform multiplication or long division to find the answer for these types of problems.
...
\begin{tikzpicture}[scale=0.6]
    ...
    \begin{scope}[rotate around={-12:(head)}]  % Change -15 to adjust tilt
        % Cap top (diamond)
        \fill[black]
            ($(head)+(-0.9,0)$) --
            ($(head)+(0,0.3)$) --
        ...
        % Tassel
       % \draw[thick] ($(head)+(0,0.3)$) -- ($(head)+(0.5,0.8)$);
        %\fill[black] ($(head)+(0.5,0.8)$) circle (0.06);
    \end{scope}
\end{tikzpicture}
