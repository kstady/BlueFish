...
\begin{remark}
Once the subtraction problem has been rewritten as addition, use the rules for adding integers to find the answer.
\end{remark}
...
Subtracting $12$ is equivalent to adding $-12.$  \\
...
\begin{remark}
When you subtract a positive number from a \emph{larger} positive number, there is no reason to rewrite the subtraction problem as
...
\textcolor{accessibleOrange}{\textbf{Answer}}  $ = \answer{-3}$
...
\textcolor{accessibleOrange}{\textbf{Answer}} $ = \answer{-12}$
...
\textcolor{accessibleOrange}{\textbf{Answer}} $ = \answer{-15}$ 
\end{problem}
...
\textcolor{accessibleOrange}{\textbf{Answer}} $ = \answer{40}$ 
\end{problem}
...
\begin{tikzpicture}[scale = 0.7, y=0.4mm, x=1mm, >=stealth]
    ...
    \draw[<->, accessibleOrange, line width=2pt] (-35,-80) -- (-35,100)
...
\textcolor{accessibleOrange}{\textbf{Answer}} $ = \answer{180} ^{\circ} \ \text{F}$ 
\end{problem}
