...
\title{Exponents and Positive Powers of 10}   % Use xmlatex ghaction or xmlatex all -s to compile when SERVE refuses to work.
\author{Kelly Stady}
...
\section*{Exponential Notation hmmmmmm}
...
\hspace{2in} \textbf{Base} $=\answer{10}$  \hspace{1in}  \textbf{Exponent} $=\answer{7}$
...
A \textbf{positive power of 10} is a power of 10 in which the exponent is a \textbf{positive integer}: $\{1, 2, 3, 4, \ldots\}.$
...
\begin{problem} A greedy CEO demanded a pay package worth \$1 trillion = $10^{12}.$  Write the value of $10^{12}.$
...
\section*{Extra Insight: The Name Behind the Search}

A \textbf{googol} is $10^{100},$ the number 1 followed by 100 zeros!  In 1920, mathematician Edward Kasner was writing a book 
called \emph{Mathematics and Imagination} and wanted to give a name to $10^{100}.$  He asked his 9-year-old nephew, Milton Sirotta, 
for a suggestion and Milton came up with the word googol.  Decades later, it inspired the name of one of the largest 
and well known tech companies: \textcolor{googleblue}{\textbf{G}}%
...
\section*{Multiplying and Dividing by Positive Powers of 10}
...
in the power of 10.  \textbf{Once you notice this relationship, you won't need to perform multiplication or long division to find the answers 
to these types of problems}.


\subsection*{Multiplying by Positive Powers of 10}
...
To multiply a number by a positive power of 10, count the number of zeros in the power of 10, then move the decimal point to the 
\wordChoice{\choice{left}\choice[correct]{right}} that many places.
...
\begin{problem}  Use the rule from the previous problem to multiply.
...
\[ 620 (100) \begin{prompt}=\answer{62000}\end{prompt} \]

\end{problem}

\subsection*{Dividing by Positive Powers of 10}
...
To divide a number by a positive power of 10, count the number of zeros in the power of 10, then move the decimal point to the 
\wordChoice{\choice[correct]{left}\choice{right}} that many places.
...
\begin{problem}  Use the rule from the previous problem to divide.  
...
this \href{https://forms.gle/BWoafF6d3UJ7PwGf9}{\textbf{Google Form}} 
to provide feedback for this lesson.
